...
from agentskills_core import SkillNotFoundError, SkillRegistry
...
class TestGetTools:
    ...
    async def test_list_skill_resources_tool(self, registry):
        ...
        listing = json.loads(await tool.ainvoke({"skill_id": "incident-response"}))
        assert listing == {
    ...
    async def test_list_skill_resources_reports_unsupported(self):
        ...
        payload = json.loads(await tool.ainvoke({"skill_id": "incident-response"}))
...
class TestToolsEdgeCases:
    ...
    async def test_multiple_skills_registered(self):
        ...
        tools = get_tools(reg)
        tool = next(t for t in tools if t.name == "get_skill_body")
        a = await tool.ainvoke({"skill_id": "skill-a"})
        b = await tool.ainvoke({"skill_id": "skill-b"})
    ...
    async def test_missing_resource_raises(self):
        ...
        with pytest.raises(ResourceNotFoundError):
            await tool.ainvoke({"skill_id": "incident-response", "name": "nonexistent.md"})
